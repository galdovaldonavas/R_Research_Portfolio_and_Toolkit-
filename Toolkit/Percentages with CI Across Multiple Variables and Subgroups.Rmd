...
# 1. Proportions for Lists of Dichotomous DVs



## Proportions for a List of Dicotomous Variables Across 1 Fixed Factor

* Expected Outcome: a dataframe with columns for percentages and 95% CI limits for all dvs and subgroups specified by the factor
...
* Input: 
  ...
  * The name of the factor used for the subgroups (my.factor), as a character string. 
...
This example is based on the project Predicting Customers Detractors 2, which uses the data.ch.i dataframe
...
prop.problems.by.condition.df$dep.variable <- factor(prop.problems.by.condition.df$dep.variable, levels = c("problem.photo", "problem.info", "problem.deadline"))

# Titles for each faceted graph representing a type of complaint
custom_titles <- c(
  problem.photo   = "Issues Loading the Photo",
  problem.info     = "Slow Information Check",
  problem.deadline = "Unmet Deadlines"
)

# Plotting the bar graph
ggplot(prop.problems.by.condition.df, aes(x = factor, y = percentage)) +
  ...
  labs(
    y = "% of Cases",
    title = "Prevalence of Chat Problems by Type of Issue Attended"
  ) +
  scale_x_discrete(labels = c ("incorrect\nitem", "other\nissue")) +
  ylim(0, 100) +  
  geom_text(
...
## 1.1. Percentages for a List of Dichotomous DVs Across 2 Fixed Factors
