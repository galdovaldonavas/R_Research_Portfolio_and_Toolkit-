...
As expected from the descriptive graphs, slow resolution complaints are significantly more frequent in chat cases involving incorrect item issues, while other types of complaints show smaller or negligible differences across conditions.
...
# 6. Thematic Analyses Within Slow Resolution Complaints

Slow resolution complaints appear to be the primary driver of the high percentage of detractors in chat cases attending incorrect item issues. To better understand the underlying issues, we simulate the presence of four specific problems within these complaints:
...
We are now ready to derive meaningful metrics from the model to understand the extent to which the presence of each problem is associated with detractors. For this, we use procedures and a reusable function from my [toolkit for logistic regression](https://github.com/galdovaldonavas/R_Research_Portfolio_and_Toolkit-/blob/main/Toolkit/Regression%20Modeling.%20Logistic%20Binomial.Rmd), already specified in section 4.2.
...
### 7.1. Bootstrapping Function to Estimate Prediction Uncertainty
...
### 7.2.2. Expressing the Impact in Relation to All Customer Service Cases
...
The results indicate that we would expect a 1.41% less detractors within all customer service cases after the correction of the issue. 
...
# 8. Following Up the Prediction of Correcting Photo Issues
...
## 8.1.Simulating the Data After the Correction
...
## 8.2. Exploration of the Change with Descriptives
...
# Testing the Significance of the Detractors Reduction
...
The results show that the decrease in detractors after correcting the photo issue was statistically significant. Interestingly, the observed reduction (about 2.4%) was even stronger than our prediction (about 1.6%).
...
The analyses indicate that correcting the photo issue in the chat service substantially reduced the percentage of detractors, both within the affected chat cases and in the overall customer service population. This means that targeted improvements in specific failure points can generate measurable gains in customer satisfaction and recommendation rates. For managers, this provides a clear business case: investing in resolving high-impact operational issues may translate into fewer detractors and a stronger brand reputation.
