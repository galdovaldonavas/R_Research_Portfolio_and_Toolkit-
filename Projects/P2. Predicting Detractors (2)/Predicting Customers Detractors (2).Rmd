...
Based on this finding, stakeholders and the research team agreed to conduct a follow-up study to identify opportunities for improvement in these cases.  

Specifically, we carried out a thematic analysis of customers’ open feedback in the NPS survey, focusing on two subsamples of chat interactions: (a) customers with incorrect item issues and (b) customers with other issues. This design allows us to isolate the effect of incorrect items within chat interactions, since chat itself is already associated with a higher share of detractors. By holding the contact channel constant, we avoid confounding the results with chat’s general negative impact.

# 4. Data Simulation

The data simulation builds on the dataset activated in [Predicting Customer Detractors (Part 1). Analyzing Contextual Factors via Logistic Regression](https://rpubs.com/galdovaldonavas/1335090).  

From that dataset, we drew random subsamples for the two scenarios of interest to avoid excessive workload in qualitative coding:  
1. Chat cases involving incorrect item issues.  
2. Chat cases involving other types of issues.  

## 4.1 Activating the Original Dataset  

To reproduce the dataset from [Part 1](https://rpubs.com/galdovaldonavas/1335090), we follow the same simulation procedure. For full details, please refer to the **Data Simulation** section in that project.
...
#Check the sample sizes for the two scenarios
with(data, table(method, reason))
