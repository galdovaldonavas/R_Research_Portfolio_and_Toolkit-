...
```
We can see that the difference in complaints is mainly associated to the difference in the presence of problems loading photos for the incorrect item cases. 
...
To confirm that the differences in condition are only drived by photo issues in the chat, we conduct chi-square tests. 
...
```{r}
# Problems loading photos in the chat: 
table.photo <- with(data.ch.i, table(problem.photo, condition))
table.photo
chisq.test(table.photo)

# Problems checking information: 
table.info <- with(data.ch.i, table(problem.info, condition))
table.info
chi <- chisq.test(table.info)
fisher <- fisher.test(table.info)
chi
fisher
...
- Reescribir: 
  - ver si hay diferencias con chi cuadrados/modelos logísticos entre las condiciones. utilizar una fórmula de agrupación, como by
- Modelar el impacto de las distintas quejas. (o si quiero solo de una - la que haya marcado diferencias)
- Recoger distintos gráficos en el Tookit
