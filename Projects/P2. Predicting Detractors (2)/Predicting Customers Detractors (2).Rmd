...
```
The chi-square tests confirm that problems uploading photos in the chat are significantly more frequent in cases experiencing incorrect item issues, versus other cases, and that this is the only significant difference we observe.

Now it is time to evaluate the impact of this problem in the likelihood to recommend our service. 


## 5.4. Association of Problems Loading Photos and Likelihood to Recommend

To evaluate the impact on the likelihood to recommend that is associated to specific cases that experience problems loading photos in the chat, we specify regression  model to predict it from the presence of the problems and the type of issues condition (which was also significant in prior analyses). 
...
m.problems.sp.solve <- glm (detractor ~ condition + problem.photo, data = data.ch.i, family = "binomial")
