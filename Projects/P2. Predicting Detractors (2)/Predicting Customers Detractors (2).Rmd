...
### Simulating the data after the project

We follow the same algorithms than for creating the data of the first semester used in the analyses, but reducing the complains slow resolution from November 1th. 
...
# Check distribution across countries
with(data.semester2, prop.table(table(complain.connect, country), margin = 2))
...
# Check distribution across contact methods
with(data.semester2, prop.table(table(complain.sp.respond, method), margin = 2))



```

For complaints about slow resolution we change the code, so that the nps scores of incorrect item cases attended by chat start decreasing on Novemeber 1th until November 30th, when the probability becomes stable in the base probability (10%)

```{r}
## --- Complaints about slow resolution ---PROBABILITY BASED ON THE DATE
...
# Create a correction factor that, starting from 1 before the correction date, progressively becomes equal to the multiplying factor until reaching the duration of the changing period
...
# Modify the probability vector based on the parameters above
...
# Check distribution by reason and method
with(data.semester2, prop.table(table(complain.sp.solve, reason, method), margin = c(2, 3)))
...
We then continue with the original fashion for the rest of the simulation: 
...
# Visualize score distribution
barplot(table(data.semester2$nps.score), ylab = "Frequency", xlab = "NPS Scores")
with(data.semester2, prop.table(table(data.semester2$nps.score)))
...
# Visualize the distribution
barplot(table(data.semester2$satisfaction), xlab = "Satisfaction Scores", ylab = "Frequency")
with(data.semester2, prop.table(table(satisfaction)))
...
#Check the results for the whole dataset
str(data.semester2)
summary(data.semester2)

```


Lastly, merge the databases to get a single dataset for the whole year
...
Calculate percentages and CI per week
...
fecha_correccion <- as.Date("2024-09-30")

ggplot(prop.detractors.by.week, aes(x = factor, y = percentage)) +
  geom_line(color = "steelblue", linewidth = 1.2) +
    geom_point(aes(size = n.cases), color = "darkred", alpha = 0.6) +
  geom_ribbon(aes(ymin = ci_lower.perc, ymax = ci_upper.perc), 
              alpha = 0.2, fill = "steelblue") +
  
  # LÍNEA VERTICAL de la corrección
  geom_vline(xintercept = fecha_correccion, 
             color = "red", linewidth = 1, linetype = "dashed") +
  
  # ANOTACIÓN de la corrección
  annotate("text", x = fecha_correccion, y = max(prop.detractors.by.week$percentage, na.rm = TRUE),
           label = "Correction of\nphoto issues", 
...
To see the percentages before and after

```{r}
# Create a dicotomous variable for before and after the correction

data.2024$moment.photo.correction <- NA_character_
data.2024$moment.photo.correction [data.2024 [data$date < "2024-10-1"] <- "before"
data.2024$moment.photo.correction [data.2024 [data$date "2024-10-1"] <- "after"
