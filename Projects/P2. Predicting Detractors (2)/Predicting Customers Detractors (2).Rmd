...
- Exploratory analysis with descriptives and visualizations  
...
Based on this finding, stakeholders and the research team agreed to conduct a follow-up to compare the customers' complaints in cases attended by chat for incorrect item issues, and cases attended by chat for other issues. study to identify opportunities for improvement in these cases. By holding the contact channel constant (chat), we avoid confounding the results with chat’s general negative impact.
...
data$complain.connect <- as.integer(rbinom(n = data.n, size = 1, prob = prob_connection))
...
data$complain.sp.respond <- as.integer(rbinom(n = data.n, size = 1, prob = prob_slow_response))
...
# Generate binary variable
data$complain.sp.solve <- as.integer(rbinom(n = data.n, size = 1, prob = prob_slow_resolution))
...
# Generate binary variable
data$complain.repeat <- as.integer(rbinom(n = data.n, size = 1, prob = prob_repeat_info))

## Other types of complaints (uniform distribution)
# Constant probability of 20% for other unspecified complaints
data$complain.other <- as.integer(rbinom(n = data.n, size = 1, prob = 0.20))
...
# ------ 5. Put aside the complaint variables to represent that this information was not available at this point

complains.df <- subset(data, select = c(id, complain.connect, complain.sp.respond, complain.sp.solve, complain.repeat, complain.other))
data<- subset(data, select = -c(complain.connect, complain.sp.respond, complain.sp.solve, complain.repeat, complain.other))
...
From that dataset, we need to see if we have enough cases for the two scenarios of interest: 
1. Chat cases involving incorrect item issues.  
2. Chat cases involving other types of issues. 


```{r}

# ------ 6. Check the number of cases contacting through different contact methods across contact reasons and countries.
...
# Prioritazing Decisions Based on Impact
