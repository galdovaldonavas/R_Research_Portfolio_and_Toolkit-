...
To confirm that the difference in the problems loading the photo between the two conditions was statistically significant, and that there were not other significant differences across conditions, we conduct chi-square tests. 

We use chi-square tests instead of logistic regression because chi-square tests are robust to having some of the categories with 0 cases, as it happens for example for our category of photo problems in other issues. 

To perform chi-squares tools, we use a reusable function from my toolkit. We first specify this function. 
...
Then we apply the function to our list of problem variables. 
...
Now it is time to evaluate the impact of this problem in the likelihood to recommend our service. 
...
However, one of the issues we have is that the factor for deadline problems presents a complete separation of scores in relation to the detractors, with all cases who present the problem being detractors. This complete separation is a major challenge for a standard logistic regression model, because the algorithm's attempt to perfectly fit the data will cause the coefficient for this factor to tend towards infinity. Let's fit a standard model to see this issue: 


```{r}
# Specify the model

trial.m.problems.sp.solve <- glm (detractor ~ problem.photo + problem.info + problem.deadline + country + condition, data = data.ch.i, family = "binomial")
summary(trial.m.problems.sp.solve)

```

As expectedd, we can see a very large standard error and a non-significant p-value for deadline problems, even though the factor's effect is clearly substantial.

To address this, we will use a penalized logistic regression, which is robust to this issue. 

### 5.4.2. Penalized Logistic Regression Model

A penalized logistic regression model, specifically with a **Lasso penalty**, is used to obtain a stable and meaningful set of coefficients that can be used for reliable probability predictions. This method adds a regularization term that prevents the coefficients from becoming infinitely large, allowing for a well-defined and convergent solution.

Let's implement this robust method using the glmnet package in R.
...
# Prepare the predictors in a matrix
x_predictors <- model.matrix(detractor ~ problem.photo + problem.info + problem.deadline + condition + country, data = data.ch.i)[,-1]

# Prepare the response variable in a vector.
y_detractor <- as.numeric(data.ch.i$detractor)

# Specify and fit a Lasso model using cross-validation.
# We use alpha = 1 for a Lasso penalty, which is effective for variable selection.
m.problems.sp.solve <- cv.glmnet(x_predictors, y_detractor, family = "binomial", alpha = 1)

# Inspect the coefficients for the optimal model.
# We retrieve the coefficients at the lambda value that minimizes cross-validation error.
coefficients <- coef(m.problems.sp.solve, s = "lambda.min")
...
# Prepare the predictors in a matrix
x_predictors <- model.matrix(detractor ~ problem.photo + problem.info + problem.deadline + condition + country, data = data.ch.i)[,-1]
# Prepare the dv in a vector
y_detractor <- as.numeric(data.ch.i$detractor)


# --- Step 2: Define a function for the bootstrapping process

# This function takes the data and a set of indices as input.
# It then fits a glmnet model on that subset of data and returns the coefficients.
fit_lasso_bootstrap <- function(data, indices) {
  
  # Sample the data using the provided indices
  d <- data[indices, ]
  
  # Prepare the data for glmnet
  x_boot <- model.matrix(detractor ~ problem.photo + problem.info + problem.deadline + condition + country, data = d)[,-1]
  y_boot <- as.numeric(d$detractor)
  
  # Fit the Lasso model with cross-validation
  cv_fit <- cv.glmnet(x_boot, y_boot, family = "binomial", alpha = 1)
  
  # Extract the coefficients from the optimal model
  coefs <- as.vector(coef(cv_fit, s = "lambda.min"))
  return(coefs)
}

# Step 3: Run the bootstrapping simulation
# We will repeat the process 1000 times to get a robust distribution of coefficients.
# Note: This step may take a few minutes to run.
set.seed(123) 
bootstrap_results <- boot(data = data.ch.i, statistic = fit_lasso_bootstrap, R = 1000)

# Step 4: Extract and display the confidence intervals
# We can use the 'boot.ci' function to calculate 95% confidence intervals for each coefficient.
# The `index` parameter corresponds to the position of the coefficient in the vector returned by our function.

# Let's check the order of our coefficients from the original model to know their indices.
original_coefs <- coef(cv.glmnet(x_predictors, y_detractor, family = "binomial", alpha = 1), s = "lambda.min")
print(original_coefs)

# Example: Get the 95% CI for 'problem.deadline'.
# Based on the output above, its index is likely 4 (after the intercept and the first two problems).
# You may need to adjust this index based on your exact output.
print(boot.ci(bootstrap_results, type = "perc", index = 4))

# Example: Get the 95% CI for 'problem.photo'.
# Its index is likely 2.
print(boot.ci(bootstrap_results, type = "perc", index = 2))
...
We can see that the problem loading photos significantly predicts that customers become detractors, to the point that after accounting for it, the differences between incorrect item cases and other cases are no longer significant. 

Since parsimony is generally preferred when non-significant predictors do not add explanatory power, we test a reduced model including only the significant predictor

```{r}

m.problems.sp.solve2 <- glm (detractor ~ problem.photo, data = data.ch.i, family = "binomial")
summary(m.problems.sp.solve2)

anova(m.problems.sp.solve, m.problems.sp.solve2, test = "Chisq")

```

The likelihood ratio test confirms that removing the condition variable does not significantly reduce model fit. Therefore, we simplify the model by excluding it.

We are now ready to derive meaningful metrics from the model to understand the extent to which the presence of detractors is associated with loading photo problems.

### 5.3.1. Creating Metrics to Interpret the Model
...
m.problems.sp.solve2.metrics.df$Label[m.problems.sp.solve2.metrics.df$Term == "(Intercept)"] <- 
  "No Photo Problems"
m.problems.sp.solve2.metrics.df$Label[m.problems.sp.solve2.metrics.df$Term == "problem.photo"] <- 
  "Problems Loading Photos"
...
ggplot(m.problems.sp.solve2.metrics.df, aes(x = Prediction, 
  ...
  geom_point(size = 3) +
    geom_text(aes(label = paste(round(Prediction,1), "%")), 
            vjust = -1, size = 3.5) + 
  labs(
    title = "Expected Detractors by Problems Loading Photos in the Chat",
    x = "% of Detractors", 
    y = "Stated Problems"
  ) +
  theme_minimal() +
