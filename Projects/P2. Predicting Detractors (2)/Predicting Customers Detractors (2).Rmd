...
To see the percentage of problems across our conditions, we apply our function (see section x)
...
In order to disentangle whether the larger number of slow resolution complaints in incorrect item cases is mainly driven by photo upload problems, we create a multicategorical variable that integrates the presence of complaints, the complaints mentioning problems loading photos, and complaints mentioning other problems. 
...
To make our model cleaner, we test whether the model can be simplified by removing this non-significant variable. 
...
We first need to build a design matrix that represents the isolated presence of each factor (and the intercept). 
...
ggplot(m.problems.sp.solve2.metrics.df, aes(x = Prediction, 
  ...
  geom_point(size = 3) +
    geom_text(aes(label = paste(round(Prediction,1), "%")), 
            vjust = -1, size = 3.5) + 
  labs(
    title = "Expected Detractors in Chat Cases By Key Problems",
    x = "% of Detractors", 
    y = "Stated Problems"
  ) +
  theme_minimal() +
...
We can see that problems uploading photos have a great impact in the probability to recommend. When these problems are present, mostly all users are expected to be detractors. In contrast, the percentage of chat detractors when this problem is only about half of the users.  
