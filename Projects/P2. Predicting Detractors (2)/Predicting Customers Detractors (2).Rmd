...
library(MASS)        # Simulate coefficients from a multivariate normal distribution
...
ggplot(m.solve2.metrics.df, aes(x = Prediction, 
  ...
  geom_point(size = 3) +
    geom_text(aes(label = paste(round(Prediction,1), "%")), 
            vjust = -1, size = 3.5) + 
  labs(
    title = "Slow Resolution Complaints in Chat By Issue Attended",
    x = "% of Complaints",
    y = "Issue Type"
  ) +
  theme_minimal() +
...
To realistically illustrate the coding of slow resolution complaints, we assume that a single complaint may mention more than one specific problem. The simulation therefore proceeds in two steps:

Number of problems mentioned (K). For each case, we first draw how many problems are mentioned (none, one, two, or three). This step allows us to keep a reasonable proportion of “unspecified” complaints (when K=0) while also controlling the degree of overlap between problems (when K>1).

Selection of specific problems. Given the value of K, we then assign the actual problems without replacement, with probabilities weighted by the case condition (incorrect item vs. other issue). This way, problems with uploading photos are much more likely to occur in incorrect item cases, whereas in other issues other types of problems tend to dominate.

This approach ensures that:
– The share of unspecified complaints remains realistic,
– Co-occurrence of problems does not inflate artificially,
– And most importantly, the excess of slow resolution complaints in the incorrect item condition can be explained primarily by difficulties with uploading photos, which is the theoretically expected pattern.
...
# Check the results
with(data.ch.i, table(problem.photo, condition))
with(data.ch.i, table(problem.info, condition))
with(data.ch.i, table(problem.deadline, condition))
with(data.ch.i, table(problem.unspecified, condition))
...
To explore how specific problems were distributed across conditions, we calculated the percentage of cases mentioning each type of problem. For comparability, we also calculated the percentage of absence of slow resolution complaints. 

All these percentages were obtained with the reusable function specified in Section X.
...
We can depict the results in a graph
...
Further plotting can help us visualize the extent to which the presence of slow resolution complaints is proportional to the difference across conditions to in the mention of problems loading photos. 


### 5.3.2.1 Exploring the Correspondence of Additional Complaints and Specific Problems

In order to disentangle whether the larger number of slow resolution complaints in incorrect item cases is mainly driven by photo upload problems, we create a multicategorical variable that integrates the presence of complaints, the complaints mentioning problems loading photos (which may include other problems), and complaints mentioning only other problems. 

```{r}
# Create a factor that integrates presence of slow resolution complaints and mentions to photo issues
data.ch.i$sp.solve.complain.problem <- NA_character_
data.ch.i$sp.solve.complain.problem [data.ch.i$no.complain.sp.solve == 1] <- "No slow resolution problems"
data.ch.i$sp.solve.complain.problem [data.ch.i$problem.photo == 1] <- "Chat not loading photos"
data.ch.i$sp.solve.complain.problem [!data.ch.i$no.complain.sp.solve == 1 & !data.ch.i$problem.photo == 1] <- "Other problems"
data.ch.i$sp.solve.complain.problem  <- factor (data.ch.i$sp.solve.complain.problem, levels = c("No slow resolution problems","Chat not loading photos", "Other problems"))


#Check if the recodification worked as expected
with(data.ch.i, table(sp.solve.complain.problem, no.complain.sp.solve, problem.photo))
...
Then we apply the function to our dependent variable representing the presence and type of slow resolution complaints segregated by our condition. 
...
Finally, we can plot the results in a bar graph with bars stacked by proportions
...
```
We can see that the difference in complaints is mainly explained by the difference in the presence of problems loading photos for the incorrect item cases. 

This difference made sense from the theoretical point of view of stakeholders, because users with incorrect items are requested to provide photos as a proof of having received the incorrect items, while this photo requirement is rare in other cases. 

We further check if the differences in the presence of the problems are statistically significant, using a logistic models.

### 5.3.3. Testing Significance of Differences Across Conditions in Slow Resolution Problems

To confirm that the difference in the problems loading the photo between the two conditions was statistically significant, and that there were not other significant differences across conditions, we conduct logistic regression models to predict the problems from the factor condition, using country as a covariate. 

```{r}

# For problems loading the photo
...
```
The chi-square tests confirm that problems uploading photos in the chat are significantly more frequent in cases experiencing incorrect item issues, versus other cases, and that this is the only significant difference we observe. (problem.info does not pass the Bonferroni Threshold)
...
To allow comparability with prior analyses, we use the presence of NPS detractors as our indicator of the likelihood to recommend. 


### 5.4.1. Preliminary Exploration of Interactions and Collinearity Issues


We conduct a mosaic plot with all the considered predictors to identify collinearity issues and interactions. Since we have many predictors, we do it in several phases:
...
```
We cannot identify relevant interactions between the factors used to predict detractors. Across different conditions and countries, the presence of each of the problems was generally associated with mostly all users being detractors. Also, this ceiling effect makes it difficult to clearly observe whether the effects of the different problems interact with each other.

We also do not identify any problems of collinearity. None of the factors show an extreme relation with any of the other factors.
...
In line with our exploratory analyses, the effect of condition is not longer significant to predict detractors once these specific problems are accounted for. Country is also a not significant factor. 

To enhance the parsimony of our model, we check the exclusion of these non significant factors. 
...
We are now ready to derive meaningful metrics from the model to understand the extent to which the presence of the problems are associated with detractors.


### 5.4.2.1 Model Metrics


To interpret the results of our binomial logistic regression, we use reusable codes from our toolkit that will help us calculate the predicted percentage of detractors when the loading problem is present and not, the differences in these predicted percentages, the relative probabilities (risk ratios), and the odds ratios.

We first need to build a design matrix that represents the isolated presence of each factor (and the intercept). Each row in the design matrix represents a scenario where one factor is present and all others are set to zero, allowing us to isolate the effect of each variable.  
...
# Extract the factor names from the model, with and without the intercept--- editions needed 
...
# We check that the factor in the matrix is specified in a similar way than in our database
str(data.ch.i)


```

The structure of the variables in the matrix aligns with the structure of the variables in the dataset. 

We then apply the matrix and our database to the function to compute the metrics and their 95% confidence intervals from logistic models. Please note that this function has been specified in section x, so we do not need to specify it here again.
...
Lastly, we visually represent the predicted proportions of detractors to clearly identify the extent to which the photo problems are associated with higher likelihoods of detractor status.
...
The points represent the predicted percentages of detractors for each scenario, with horizontal whiskers showing the 95% confidence intervals.

We can see that problems uploading photos have a strong impact on the likelihood of being a detractor. When these problems are present, nearly all users are expected to become detractors. In contrast, in the absence of this problem, only about half of the users are predicted to be detractors.

## 5.5. Impact of Problems Loading Photos Across All Users

We will simulate how the correction of each of the problems identified would impact the reduction of detractors 


### 5.5.1. Simulating Coefficients of the Relation between Problems and Detractors

For this simulation we will: 
- Simulate a sample distribution of our regression coefficients based on our model about the effect of the problems in customers becoming detractors. 
- Simulate a sample distribution of the predicted probabilities of being detractor using a bootstrapping technique that uses the simulation of the distribution of coefficients. 
...
predictor.names <- names(original.coefficients) 
predictor.names <- predictor.names [-1]
...
simulated_coefficients.df <- data.frame(mvrnorm(n = my.n, 
...
for (group in groups) {
  ...
  # Create a list to store the results for a group 
  ...
  for (i in 1:my.n) {
    ...
    # Selecting the simulated coefficients for the sample
    sample.coefficients <- simulated_coefficients.df [i , ]
  
  
    # Selecting a bootstrap sample from our sample
    ...
    prob.success <- rep(NA, nrow(bootstrap.sample))
      ...
      predictor.effects.vector <- rep(NA, length(predictor.names))
      p <- 1
      
      # Loop for each predictor
       for (name in predictor.names) { 
         
         #Calculate the effect per each predictor
       ...
       # Calculate the probability of each case
        prob.success [c] <- 1 / (1 + exp (-1 * (sample.coefficients[1] + sum(predictor.effects.vector))))
    
      } # Close the loop for each case in the bootstrapped sample

  # Calculate the proportion of successes in the bootstrapped sample
  proportion.sample <- mean(prob.success)
  ...
  estimate.probability <- median (sampling.distribution)
  lower.ci.estimate <- quantile(sampling.distribution, probs = 0.025)
  upper.ci.estimate <- quantile(sampling.distribution, probs = 0.975)
  
  # Create a name for the sublist of results for each subgropu
  name.group.results <- paste0(group, "results")
  
  # Store the results
  final.results [name.group.results] <- list(estimate = estimate.probability, lower.ci = lower.ci.estimate, upper.ci = upper.ci.estimate, bootstrap.sampling.dist = sampling.distribution)
...
```{r}
sim.predictors.present <- f.bootstrap.predictions (my.n = 10,
...
f.bootstrap.predictions <- function(my.n, my.model, my.data, my.factor) {
  ...
  for (group in groups) {
    ...
    for (i in 1:my.n) {
      ...
      # Bootstrap sample
      bootstrap.indexes <- sample(1:nrow(subsample), nrow(subsample), replace = TRUE)
      bootstrap.sample <- subsample[bootstrap.indexes, ]
      ...
      if (i == 1 & group == groups[1]) {
        print(paste("Nombres de coeficientes:", names(sample.coefficients)))
        print(paste("Nombres de predictores:", predictor.names))
        print(paste("Estructura de bootstrap.sample:", class(bootstrap.sample)))
      }
      
      # A loop for each case
      for (c in 1:nrow(bootstrap.sample)) {
        predictor.effects.vector <- numeric(length(predictor.names))
        
        for (p in 1:length(predictor.names)) {
          name <- predictor.names[p]
          # Extraer valores de forma más robusta
          coef_value <- as.numeric(sample.coefficients[[name]])
          data_value <- as.numeric(bootstrap.sample[[name]][c])
          predictor.effects.vector[p] <- coef_value * data_value
        }
        
        intercept <- as.numeric(sample.coefficients[1])
        linear_predictor <- intercept + sum(predictor.effects.vector)
        prob.success[c] <- 1 / (1 + exp(-linear_predictor))
        
        # Debug check
        if (is.na(prob.success[c])) {
          print(paste("NA en caso", c, ":", linear_predictor))
        }
      }
      
      # Verificar prob.success antes de mean()
      if (any(is.na(prob.success))) {
        print(paste("ADVERTENCIA: NAs en prob.success para iteración", i))
        print(summary(prob.success))
      }
      
      proportion.sample <- mean(prob.success, na.rm = TRUE)  # na.rm añadido temporalmente
      sampling.distribution[i] <- proportion.sample
    }
    ...
    name.group.results <- paste0(group, "_results")
...
```{r}
sim.predictors.present <- f.bootstrap.predictions (my.n = 5000,
                        my.model = m.problems.sp.solve2, 
                        my.data = data.ch.i, 
                        my.factor = "condition")
```
...
We first extract the covariance matrix of the model that especificies these relations

```{r}
matriz_vcov <- vcov(m.problems.sp.solve2)
matriz_vcov
```

We can see that the diagonal corresponds with the variance of the coeficients (if we calculate the square root we obtain the standard error of the coefficients). Outside the diagonal we can see the covariances that represent the relation in the estimation errors of the coefficients, which we can see is minimal for all problems. 

Next, we generate a set of random coefficients, using the matrix to simulate them. 

```{r}

# Extract coefficients from the original model
original_coefficients <- coef(m.problems.sp.solve2)

# Set the n for the number of simulations
n_simulations <- 10000 

# Generate a matrix where each line is a set of simulated coefficients
simulated_coefficients.df <- data.frame(mvrnorm(n = n_simulations, 
                                 mu = original_coefficients, 
                                 Sigma = matriz_vcov))

#Update the variable names
old.names <- names (simulated_coefficients.df)
new.names <- paste0 ("coef.", old.names)
names(simulated_coefficients.df) <- new.names
head(simulated_coefficients.df)
```

### 5.5.2. Simulating The Presence of the Problems Across Conditions

To ponderate the effects simulated above by the presence of the problems, we first need to simulate the presence based on our data. 

For that, we extract the prevalence of the problems in our sample and the confidence intervals of those prevalences. It is important to note that for the problem loading photos and for problems checking the information, differed across conditions, so we will use the specific percentages for each conditon
...
dv.list = c("problem.photo", "problem.info", "problem.deadline")

prop.problems.by.condition.df <- f.prop.dicdvl.1f (my.data = data.ch.i,
                  my.dv.list = dv.list,
                  my.factor = "condition")

prop.problems.by.condition.df


```


For the deadlines problems there were not significant differences, so we will use the general prevalence of this problem in the whole sample. We first calculate it and input its value for both conditions in the above results. 

```{r}
# --- 1. Calculation of the percentage and 95% CI for problem.deadlines

# Extract a frequency table
t.problem.deadline <- with(data.ch.i, table(problem.deadline))
t.problem.deadline

# Calculate the proportion and confidence interval based on the table
prop.problem.deadline <- binom.test(x = t.problem.deadline[2], n = sum(t.problem.deadline))
prop.problem.deadline


# --- 2. Update the prior dataframe of percentages with the percentage results for deadline problems
# Save the percentages in a new dataframe
prevalence.par.df <- prop.problems.by.condition.df

#Convert them to proportions
prevalence.par.df [,3:5] <- prevalence.par.df [,3:5]/100
names (prevalence.par.df)[names(prevalence.par.df) %in% c("percentage", "ci_lower.perc", "ci_upper.perc")] <- c("prop", "ci.lower.prop", "ci.upper.prop")



#Update the results for deadline problems
prevalence.par.df [5:6, 3] <- prop.problem.deadline$estimate 
prevalence.par.df [5:6, 4] <- prop.problem.deadline$conf.int [1] 
prevalence.par.df [5:6, 5] <- prop.problem.deadline$conf.int [2] 
prevalence.par.df
...
For each problem across each condition, we calculate the beta and alpha values for the simulation of the prevalence, and keep it in a dataframe. 

```{r}
# Extract the variance of the percentages based on the confidence intervals
prevalence.par.df$var.prop <- ((prevalence.par.df$ci.upper.prop - prevalence.par.df$ci.lower.prop) / (2 * 1.96)) ^ 2

# Calculate the alpha value
prevalence.par.df$alpha <- prevalence.par.df$prop * ((prevalence.par.df$prop * (1 - prevalence.par.df$prop) / prevalence.par.df$var.prop) - 1)

# Calculate the beta value
prevalence.par.df$beta <- (1 - prevalence.par.df$prop) * ((prevalence.par.df$prop * (1 - prevalence.par.df$prop) / prevalence.par.df$var.prop) - 1)
...
Now we calculate the prevalences in a simulation of 10000

```{r}

# Function to perform the beta distributions for the prevalence

f.beta.simulation <- function (my.n, my.data, my.dv, my.alpha, my.beta) {
  
  # Create a list to keep the results
  results <- list ()
  p <- 1
  
  # Extract the names of the dvs
  dv.names <- unique (my.data[[my.dv]])
  
  # Open a loop for each dv
  for (dv in dv.names) {
    
    #Extract the parameters
    current.alpha <- my.data[my.data[[my.dv]] == dv, my.alpha]
    current.beta <- my.data[my.data[[my.dv]] == dv, my.beta]

    # Simulate a vector that indicates presence of each dv
    simulated_vector <- rbeta(n = my.n,
                              shape1 = current.alpha, 
                              shape2 = current.beta)
    
    # Save the results
    results [[p]] <- simulated_vector
    p = p + 1
  }
  
  # Bind the results in columns of a single daframe
  results.df <- data.frame(do.call(cbind, results))
  column.names <- paste0("prev.", dv.names)
  names(results.df) <- column.names
...
And we use the function to simulate the prevalences across the two conditions

```{r}
# ---- 1. For incorrect item issues

# Create a subset dataframe with the incorrect item parameters
ii.prevalence.par.df <- prevalence.par.df [prevalence.par.df$factor == "chat_incorrect_item", ]
ii.prevalence.par.df 

# Apply the function to perform beta distributions
ii.prevalence.simulation.df <- f.beta.simulation (my.n = n_simulations, 
                                                  my.data = ii.prevalence.par.df,
                                                  my.dv = "dep.variable", 
                                                  my.alpha = "alpha", 
                                                  my.beta = "beta")

# Merge the simulation of the prevalence and the simulation of the coefficients
ii.simulation <- cbind(ii.prevalence.simulation.df, simulated_coefficients.df)
...
# Create a subset dataframe with the incorrect item parameters
oi.prevalence.par.df <- prevalence.par.df [prevalence.par.df$factor == "chat_other_issues", ]
oi.prevalence.par.df 

# Apply the function to perform beta distributions
oi.prevalence.simulation.df <- f.beta.simulation (my.n = n_simulations, 
                                                  my.data = oi.prevalence.par.df,
                                                  my.dv = "dep.variable", 
                                                  my.alpha = "alpha", 
                                                  my.beta = "beta")

# Merge the simulation of the prevalence and the simulation of the coefficients
oi.simulation <- cbind(oi.prevalence.simulation.df, simulated_coefficients.df)
...
We can proceed to estimate the probabilities for the present situation

```{r}
# 1.--- Estimate present probabilities of detractors 

# 1.1 - For incorrect item issues
ii.simulation$present.prob <- with (ii.simulation, {
  1 / (1 + exp (-1*(coef.X.Intercept.+
                      prev.problem.photo * coef.problem.photo +
                      prev.problem.info * coef.problem.info + 
                      prev.problem.deadline * coef.problem.deadline)))
})

# 1.2 - For other issues
oi.simulation$present.prob <- with (oi.simulation, {
  1 / (1 + exp (-1*(coef.X.Intercept.+
                      prev.problem.photo * coef.problem.photo +
                      prev.problem.info * coef.problem.info + 
                      prev.problem.deadline * coef.problem.deadline)))
...
# 2.--- Estimate predicted probabilities after correcting for photo problems 

# 1.1 - For incorrect item issues
ii.simulation$predicted.prob.photo <- with (ii.simulation, {
  1 / (1 + exp (-1*(coef.X.Intercept.+
                      prev.problem.photo * 0 +
                      prev.problem.info * coef.problem.info + 
                      prev.problem.deadline * coef.problem.deadline)))
})

# 1.2 - For other issues
oi.simulation$predicted.prob.photo <- with (oi.simulation, {
  1 / (1 + exp (-1*(coef.X.Intercept.+
                      prev.problem.photo * 0 +
                      prev.problem.info * coef.problem.info + 
                      prev.problem.deadline * coef.problem.deadline)))
...
```{r}
f.detractors <- with(data.ch.i, table(detractor, condition))
prop.f.detractors <-prop.table(f.detractors, margin = 2)
f.detractors
prop.f.detractors
...
SEGUIR AQUÍ: Preguntar a AI si es correcto multiplicar por los coeficients por las probabilidades, en lugar de por la presencia o no de casos. Parece que lo que estamos simulando son posibles resultados poblacionales. 
...
# Inicializa un data frame para guardar los resultados finales
resultados_simulacion <- data.frame(
  reduccion_photo = numeric(10000),
  reduccion_info = numeric(10000),
  reduccion_deadline = numeric(10000)
)

# El bucle de simulación
for (i in 1:10000) {
  
  # Paso 1: Generar un conjunto de coeficientes para esta iteración
  # (Ya hiciste esto y lo guardaste en 'simulated_coefficients')
  coef_simulados <- simulated_coefficients[i, ]
  
  # Paso 2: Generar un valor de prevalencia simulado para cada problema
  #         y cada condición, usando los parámetros alpha y beta.
  prev_photo_incorrect_sim <- rbeta(1, prevalence.par.df$alpha[1], prevalence.par.df$beta[1])
  prev_photo_other_sim <- rbeta(1, prevalence.par.df$alpha[2], prevalence.par.df$beta[2])
  # Repite para los otros problemas...
  prev_info_incorrect_sim <- rbeta(1, prevalence.par.df$alpha[3], prevalence.par.df$beta[3])
  prev_info_other_sim <- rbeta(1, prevalence.par.df$alpha[4], prevalence.par.df$beta[4])
  prev_deadline_sim <- rbeta(1, prevalence.par.df$alpha[5], prevalence.par.df$beta[5])
  
  # Ahora que tienes los coeficientes y las prevalencias simuladas,
  # puedes pasar al cálculo.
  
  # --- Paso 3: Cálculo de la probabilidad de detractor en el ESCENARIO ACTUAL ---
  # Calcula la probabilidad en el subgrupo 'incorrect items'
  prob_detractor_i_actual <- 1 / (1 + exp(-(
    coef_simulados["(Intercept)"] +
    coef_simulados["problem.photo"] * prev_photo_incorrect_sim +
    coef_simulados["problem.info"] * prev_info_incorrect_sim +
    coef_simulados["problem.deadline"] * prev_deadline_sim
  )))
  # ...y para el subgrupo 'other issues'
  prob_detractor_o_actual <- 1 / (1 + exp(-(
    coef_simulados["(Intercept)"] +
    coef_simulados["problem.photo"] * prev_photo_other_sim +
    coef_simulados["problem.info"] * prev_info_other_sim +
    coef_simulados["problem.deadline"] * prev_deadline_sim
  )))
  
  # Calcula el porcentaje total de detractores actual (ponderando)
  porc_detractor_total_actual <- (prob_detractor_i_actual * 0.10) + (prob_detractor_o_actual * 0.30)
  
  # --- Paso 4: Calcular la probabilidad en los ESCENARIOS CORREGIDOS ---
  # Escenario 1: Corregir problem.photo (prevalencia = 0)
  prob_detractor_i_photo <- 1 / (1 + exp(-(
    coef_simulados["(Intercept)"] +
    coef_simulados["problem.photo"] * 0 + # Aquí la prevalencia es 0
    coef_simulados["problem.info"] * prev_info_incorrect_sim +
    coef_simulados["problem.deadline"] * prev_deadline_sim
  )))
  prob_detractor_o_photo <- 1 / (1 + exp(-(
    coef_simulados["(Intercept)"] +
    coef_simulados["problem.photo"] * 0 + # Aquí la prevalencia es 0
    coef_simulados["problem.info"] * prev_info_other_sim +
    coef_simulados["problem.deadline"] * prev_deadline_sim
  )))
  porc_detractor_total_photo <- (prob_detractor_i_photo * 0.10) + (prob_detractor_o_photo * 0.30)
  
  # ...y lo mismo para los escenarios de corrección de problem.info y problem.deadline.
  
  # --- Paso 5: Calcular y guardar las reducciones porcentuales ---
  reduccion_photo <- ((porc_detractor_total_actual - porc_detractor_total_photo) / porc_detractor_total_actual) * 100
  resultados_simulacion$reduccion_photo[i] <- reduccion_photo
  # ...y lo mismo para las otras reducciones.
}

# Al final del bucle, los percentiles 2.5 y 97.5 de las columnas en 'resultados_simulacion'
# te darán tu intervalo de confianza del 95%.
...
To understand what is the impact that this photo issue is having on the total detractors we have in customer service, we have to consider two pieces of information: 
- Estimates of the extent to which this problem is present.
- The estimated relation between the presence of this problem and probability to become detractors (seen in the prior section)
-  




* Estimación de % of detractors en incorrect item issues y otros casos si el problema no estuviera presente: 44.7% en ambos, si, pero de cuánto a cuánto se reduce en cada condición. 

Creo que la solución va a ser simular x casos con la probabilidad de que el problema esté presente (obtenido dle modelo de presencia y su CI), y x casos la diferencia porcentual del efecto (con sus CI). Estos valores se multiplicarán uno por el otro. De esta manera tendremos los intervalos de confianza
...
- Recoger distintos gráficos en el Tookit. - Guardar estas gráficas y procedimientos: para cuando hay variables multicategóricas, grafico con y sin CI
...
write.xlsx(data.ch.i, "Predicting Detractors 2. data.ch.i.xlsx")
write.xlsx(data.complaints, "Predicting Detractors 2. data.complaints.xlsx")


```
 

Se me ocurre: - Primero utilizar una simulación con bootstraping the x casos (e.g., 100000) donde simulemos la presencia del problema en cada subgrupo (i.e., en el subgrupo de chat incorrect items, y en el subgrupo de chat other issues). Para ello usaríamos el porcentaje de la presencia estimado y su CI. - Simular también el efecto asociado para cada uno de los casos anteriores, usando la estimación de la reducción de detractores y su CI. - Multiplicar para cada uno de los 10000 casos el efecto por la presencia, obteniendo de esta manera una distribución de nuestras estimaciones de la probabilidad de reducir detractores dentro de cada subgrupo (e.g., dentro de chat incorrect items). Así podemos obtener la reducción de detractores dentro de ese subgrupo y su intervalo de confianza. - Ponderar ese número por el porcentaje de casos que ese subgrupo representa en el total de casos de customer service, por ejemplo por .10 para chat incorrect item issues. 


Se me ocurre: - 1. Para tener en cuenta la prevalencia de los problemas: utilizar una simulación con bootstraping the x casos (e.g., 100000) donde simulemos la presencia de cada problema en cada subgrupo (i.e., en el subgrupo de chat incorrect items, y en el subgrupo de chat other issues). Para cada problema,  usaríamos el porcentaje de la presencia estimado y su CI obtenidos a partir de un modelo de regresion logística en el que se predigan los detractores a partir del problema y otros factores contextuales (país, contexto) que sean significativos en dicha predicción.  

 - 2. Para tener en cuenta el efecto de los problemas en los detractores. Utilizar mi modelo de regresión para calcular el incremento porcentual de detractores asociados a la presencia de cada problema, y sus ci. Simular también 10000 casos de esas estimaciones de incremento porcentual en detractores. 
 
 - 3. Para obtener la reducción porcentual en cada contexto que haya sido significativo en la prevalencia de cada probema: Multiplicar para cada uno de los 10000 casos simulados de la presencia por uno de los 10000 casos simulados para el efecto de la presencia del problema en los detractores. Obtener con ello la reducción de detractores dentro de ese subgrupo y su intervalo de confianza. 
 
 - 4. Para tener en cuenta el número de casos totales asociados a cada contexto: Ponderar el resultado anterior por el porcentaje de casos que ese subgrupo representa en el total de casos de customer service, por ejemplo por .10 para chat incorrect item issues. 
...
#####
# 1. Inputs: estimaciones conocidas
#####

# Presencia del problema de fotos (% y CI por subgrupo)
# Ejemplo: 40% en incorrect items (CI 30–50%), 5% en other issues (CI 1–10%)
p.incorrect.mean <- 0.40
p.incorrect.low  <- 0.30
p.incorrect.high <- 0.50

p.other.mean <- 0.05
p.other.low  <- 0.01
p.other.high <- 0.10

# Tamaño relativo de cada subgrupo en el total de casos
w.incorrect <- 0.10  # 10% de los casos de CS
w.other     <- 0.30  # 30% de los casos de CS

# Efecto estimado sobre detractores (diferencia en probabilidad de ser detractor)
# Ejemplo: +45pp (CI: +35 a +55)
effect.mean <- 0.45
effect.low  <- 0.35
effect.high <- 0.55
...
#####
# 2. Preparar funciones auxiliares
#####

# Función para obtener parámetros de una Beta a partir de media e intervalo
beta_params <- function(mean, low, high, conf = 0.95) {
  # aproximación usando var ≈ ((upper-lower)/(2*1.96))^2
  se <- (high - low) / (2 * qnorm(1 - (1 - conf)/2))
  var <- se^2
...
# Simulación con Beta
sim_beta <- function(mean, low, high, n = 10000) {
  pars <- beta_params(mean, low, high)
  rbeta(n, pars$alpha, pars$beta)
}

# Simulación del efecto (diferencia de probabilidad), usando normal truncada en [0,1]
sim_effect <- function(mean, low, high, n = 10000) {
  se <- (high - low) / (2 * qnorm(0.975))
  vals <- rnorm(n, mean, se)
  vals[vals < 0] <- 0
  vals[vals > 1] <- 1
  return(vals)
}
...
# Presencia del problema en cada subgrupo
p.incorrect <- sim_beta(p.incorrect.mean, p.incorrect.low, p.incorrect.high, n.sim)
p.other     <- sim_beta(p.other.mean, p.other.low, p.other.high, n.sim)

# Efecto en detractores
effect <- sim_effect(effect.mean, effect.low, effect.high, n.sim)

# Reducción esperada de detractores en cada subgrupo
red.incorrect <- p.incorrect * effect
red.other     <- p.other * effect

# Reducción total ponderada
red.total <- w.incorrect * red.incorrect + w.other * red.other
...
impact.incorrect <- summary_stats(red.incorrect)
impact.other     <- summary_stats(red.other)
impact.total     <- summary_stats(red.total)
