---
title: "Predicting Customer Detractors (Part 1). Analyzing Contextual Factors Via Logistic Regression"
output:
  html_document:
    df_print: paged
...
## 3.2. Simulating Underlying Customer Complaints
...
Please note that these variables represent underlying complaints that were not available at this stage of the investigation. We use them only to simulate NPS and satisfaction scores and store them in a separate dataframe to reflect their unavailability. They become visible only after thematic coding in the follow up project [Predicting Customer Detractors (Part 2): Opportunity Analysis from Open Feedback](https://rpubs.com/galdovaldonavas/1335090)).
...
with(data.complaints, prop.table(table(complain.connect, country), margin = 2))
...
data.complaints$complain.sp.respond <- rbinom(n = data.n, size = 1, prob = prob_slow_response)
# Check distribution across contact methods
with(data.complaints, prop.table(table(complain.sp.respond, method), margin = 2))
...
# Generate binary variable
data.complaints$complain.sp.solve <- rbinom(n = data.n, size = 1, prob = prob_slow_resolution)
# Check distribution by reason and method
with(data.complaints, prop.table(table(complain.sp.solve, reason, method), margin = c(2, 3)))
...
# Generate binary variable
data.complaints$complain.repeat <- rbinom(n = data.n, size = 1, prob = prob_repeat_info)
# Check distribution across contact reasons
with(data.complaints, prop.table(table(complain.repeat, reason), margin = 2))


## Other types of complaints (uniform distribution)
# Constant probability of 20% for other unspecified complaints
data.complaints$complain.other <- rbinom(n = data.n, size = 1, prob = 0.20)
# Check distribution
with(data.complaints, prop.table(table(complain.other)))

```


## 3.3. Simulating Net Promoter Score (NPS) Outcomes
...
## 3.4. Simulating Satisfaction Scores
...
### 3.4.1. Introducing Missing Data for Realism
...
## 4.1. Assessing Convergent Validity
...
## 4.1.1. Examining Variable Distributions
...
### 4.1.2. Spearman Correlation Analysis
...
# 5. Exploratory Analysis of Factor Interactions
...
## 5.1. A Reusable Function for Interaction Heatmaps
...
## 5.2. Heatmap of Categorical NPS
...
## 5.4. Heatmap of Satisfaction Scores
...
## 5.5. Confirming Convergence Across Metrics
...
# 6. Statistical Modeling of Detractors
...
## 6.1. Defining Model Reference Categories
...
## 6.2. Rationale for Rejecting Linear Regression
...
## 6.3. Challenges with Ordinal Logistic Regression
...
### 6.3.2. Testing the Proportional Odds Assumption
...
##### 6.3.2.1 Adding a Theoretically-Grounded Interaction
...
##### 6.3.2.2 Attempting a Non-Proportional Odds Model
...
##### 6.3.2.3. Diagnosing the Source of the Violation
...
## 6.4. Binomial Logistic Regression Model
...
#### 6.4.1.Deriving Actionable Interpretation Metrics
...
# 7. Business Impact and Prioritization
...
However, we do not yet know the specific issues customers encounter within each contextual factor. This will be explored in the next study case: [Predicting Customer Detractors (Part 2): Opportunity Analysis from Open Feedback](https://rpubs.com/galdovaldonavas/1345416)
