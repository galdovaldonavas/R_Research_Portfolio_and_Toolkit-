...
## 5.3. Further Specificity Coding and Analyzing Key Complaints

Once we identified that complaints about slow resolution speed were driving the difference between incorrect item issues and other issues within chat cases, it became important to further identify and code for the specific problems users were raising. This would allow us to evaluate whether these problems represented opportunities for improvement.
...
### 5.3.1. Coded Data for Specific Slow Resolution Complaints

To illustrate this coding step, we simulate a distribution of specific complaints depending on whether the case involved incorrect item issues or other issues, and on whether the case already complained about slow resolution issues:
* Presence of complaints about slow resolution: 
  * Incorrect item issues: 
    * 70% of complaints about difficulties attaching the photo to the chat.
    * 20% of complaints about slow procedures checking the information. 
    * 10% of unspecific or ambiguous complaints about slow resolution. 
  * Other issues: 
    * 6% of complaints about difficulties attaching the photo to the chat. 
    * 64% of complaints about slow procedures checking the information.
    * 30% of unspecific or ambiguous complaints about slow resolution.
*For cases that did not complain about slow resolution we assume no complains in any of the above categories.  
  
  
```{r}

# Save the levels for the specific problems in slow resolution complaints
levels.sp.solve <- c("difficulties_loading_photos", "slow_information_check", "unespecific")
...
# Introduce values, specifying no complains of slow resolution
subset.no.c$sp.solve.problem <- "no complaints about resolution"

# Group again the subsets
data.ch.i <- bind_rows(subset.ii, subset.oi, subset.no.c)

# Display distribution of coded problems by condition
