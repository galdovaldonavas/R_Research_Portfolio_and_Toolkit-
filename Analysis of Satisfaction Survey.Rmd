...
```{r}
#Saving the model as my.model
my.model <- binomial.model.c

# Extracting the coefficients
coefs <- coef(my.model)
intercept <- coefs[1]

#Vector with betas, in which the beta for the intercept column is 0
coefs.f <- coefs
coefs.f[1] <-0

#objects for the intercept and factor coefficients
intercept
coefs.f
coefs
...
#calculating the probability of the intercept
p0 <- 1/(1+e^((-1)*intercept))
p0

#calculating the probability of each factor

p.factors <- 1/(1 + e^((-1)*(intercept + coefs.f )))
p.factors 


#calculating the differences in probability from intercept versus probability of each factor
p.dif <- p.factors - p.factors[1]
p.dif


```

Ahora lo hago con predict

```{r}
####
#Design a matrix that define the presence of the factors
####

# 1. We extract the names of the factors from the model
f.names <- names(coef(my.model))
f.names <- f.names [-1] #to eliminate the intercept

# 2. We create a dataframe with an identity matrix with the length of our factors 
my.matrix.df <- as.data.frame(diag(length(f.names)))
names(my.matrix.df) <- f.names
my.matrix.df$f_name <- f.names
my.matrix.df

# 3. Manually specify the case for your interactions
my.matrix.df$chat[my.matrix.df$f_name =="chat_incorrect_item"] <-1
my.matrix.df$incorrect_item[my.matrix.df$f_name =="chat_incorrect_item"] <-1
my.matrix.df


# 4. We add a column for the intercept
my.matrix.df$intercept <- 1
my.matrix.df

# 5. Reorganization of the columns order
my.matrix.df <- my.matrix.df[, c("f_name", "intercept", f.names)]
my.matrix.df

# 6. Make sure the variables are expressed like in the original dataset. We do this with country
my.matrix.df$country <- NA_character_
my.matrix.df$country[my.matrix.df$countryDrinkland==1] <- "Drinkland"
my.matrix.df$country[!(my.matrix.df$countryDrinkland==1)] <- "Eatland"
...
### 2.3.3. Results Interpretation: Extent With Which Each CS Factors is Associated with Detractors

This section uses the information from the logistic regression model above to provide metrics of how much each factor is associated with customers becoming detractor, including metrics like the odds ratios, the relative proportion, and the absolute percentage change. For this last metric, we will use use parametric simulations, for which we need to create a matrix about cases that represent the individual presence of the factors. 

We start specifying this matrix, where we have the variables in our model as in the original dataset, a case representing the intercept with 0s for all variables, and a case for the individual presence of each factor. 
...
We create a formula to obtain the odds ratios, relative proportions, predicted proportions, and absolute changes from the predicted proportions from the binomial model and the specified matrix. 

The formula also provides the 95% confidence intervals for these metrics. For calculating the confidence intervals for the absolute changes from the predicted proportions, we employ parametric bootstrap simulations. 
...
f.metrics.binomial <- function(my.model, my.matrix){
  ...
  pred <- predict(my.model, newdata = my.matrix.df, type = "link", se.fit = TRUE)
  ...
  perc_diff <- colMeans(diffs)
  lower_perc_diff  <- apply(diffs, 2, quantile, probs = 0.025)
  higher_perc_diff <- apply(diffs, 2, quantile, probs = 0.975)
...
OBTAINING THE PREDICTED PROBABILITIES AND THEIR CONFIDENCE INTERVALS


```{r}
#Calculate log-odds and standard errors
pred <- predict(my.model, newdata = my.matrix.df, type = "link", se.fit = TRUE)
...
#Transform to probabilities, using a function
f.inv_logit <- function(x) {1 / (1 + exp(-x))}
...
OBTAINING THE DIFFERENCES OF THE EXPECTED PERCENTAGES AND THEIR CONFIDENCE INTERVALS
We employ parametric bootstrap simulations to estimate the differences in detractor percentages linked to the presence of key factors
...
# Transform logg-odds to expected probabilities
simulated_probs <- f.inv_logit(simulated_log_odds)

# 4. Calculate differences from the first column that correspond to the intercept probabilities
diffs <- apply(simulated_probs, 2, function(col){
  col - simulated_probs[[1]]
})

# 5. Calculate the means of the differences and 95% CI with percentiles
perc_diff <- colMeans(diffs)
lower_perc_diff  <- apply(diffs, 2, quantile, probs = 0.025)
higher_perc_diff <- apply(diffs, 2, quantile, probs = 0.975)
...
# 2. Simular log-odds muchas veces (por ejemplo, 10000 veces)
set.seed(123)
simulated_log_odds <- matrix(
  rnorm(10000 * length(pred$fit), mean = pred$fit, sd = pred$se.fit),
  ncol = length(pred$fit), byrow = TRUE
)

# 3. Transformar a probabilidad
simulated_probs <- f.inv_logit(simulated_log_odds)

# 4. Calcular diferencias respecto al intercepto (primera fila)
diffs <- sweep(simulated_probs, 1, simulated_probs[,1], "-")

# 5. Calcular percentiles para IC al 95%
perc_diff <- colMeans(diffs)
lower_perc_diff  <- apply(diffs, 2, quantile, probs = 0.025)
higher_perc_diff <- apply(diffs, 2, quantile, probs = 0.975)
...
f.metrics.binomial <- function(my.model){
  ...
  # Exponentiation of the coefficients and CIs to obtain odds ratios
  or <- exp(coefs)
  or_ci <- exp(ci)
