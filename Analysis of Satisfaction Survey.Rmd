...
data$satisfaction <- floor(rnorm(data.n, mean = 3, sd=0.5) + 0.9* (1+scale(data$nps.score)))
...
### 2.2.1. Satisfaction scores Heatmap
...
# 1.Obtaining the first line of the dataset: results for the blocks and their cross with columns

# 1.1. We create a loop for each block
for (b in factor.blocks.levels){
  
  ## 1.2.Label the line to put the result for the block 
  row.label.b <- paste0("Total: ", b)
  
  ## 1.3.Calculate the total for the block
  b.subset <- my.data[ my.data[["factor.blocks"]]== b, ]
  b.total <- my.function(b.subset[["my.dv"]], na.rm=TRUE) 
  
  ## 1.4.Calculate the results for the block desagregated by the columns factor
  ...
  ## 1.5. Save the results for the block in a named vector
  results [[k]] <- c(group = row.label.b, overall= b.total, setNames(object=bc.crossed, factor.column.levels))
  k<- k+1 
 
  
      # 2.The remaining lines of the dataset corresponding to each block.
      #each line will shows results for blocks crossed with rows, 
      #and within each of those lines, the cross of both factors with the column factor:     
  
      ## 2.1. Create a sub-loop for rows, within the prior loop of blocks (see 1.1)
      for(r in factor.rows.levels){ 
        ...
        ## 2.5. Save the results of each line, corresponding to each row level, in a named vector
...
# 1. Create the first line of the dataset, introducing a general block that does not desagregate results by the block factor
    #this line will contain the value for the total of the whole sample, and this total desagregated by the column factor

##1.1. Create the label for specifying we refer to results of the whole sample
row.label.t <- paste0("Total all sample")

##1.2. Calculate the total results for the whole sample
if(nrow(my.data)>0){
  total <- my.function(my.data[["my.dv"]], na.rm=TRUE)
} else {stop("The dataset is empty")
}

##1.3. Calculate the total results for the columns factor
...
## 1.4. Save the results of this line in a named vector
results[[k]] <- c(group = row.label.t, overall = total, setNames(object= c.total, factor.column.levels) ) 
k <- k+1

# 2. Create additional lines for the general block
      #each line will shows results for the total of each level of the row factor
      #and additional columns for that level of the row factor across the levels of the colums factor 

##2.1. Create a loop for each level in the row factor
for (r in factor.rows.levels){
  
  ##2.2. Create a label for each line
  row.label.r <- paste0(r)
  
  ##2.3. Calculate the total for each level of the row factor
  r.total.subset <- my.data[my.data[["factor.rows"]]==r, ]
  r.total <- my.function(r.total.subset[["my.dv"]], na.rm=TRUE)
  
  ##2.4. Calculate the value for each level in the row factor
...
# 4. We agregate the results in a dataframe  
