...
Next, we create a formula to obtain the odds ratios, relative proportions, predicted proportions, and absolute changes from the predicted proportions from the binomial model and the specified matrix. 

The formula also provides the 95% confidence intervals for these metrics. For calculating the confidence intervals for the differences in the predicted proportion, we employ parametric bootstrap simulations. 
...
f.metrics.binomial <- function(my.model, my.matrix){
  ...
  results <- data.frame(
    Term = names(rp),
    Prediction = as.numeric(predicted_prob),
    Lower_Prediction = as.numeric(lower_predicted_prob),
    Upper_Prediction = as.numeric(upper_predicted_prob),
    Predicted_Change = as.numeric(prob_diff),
    Lower_Predicted_Change = as.numeric(lower_prob_diff),
    Upper_Predicted_Change = as.numeric(upper_prob_diff),
...
Now we are ready to apply the function to our model: 
...
For better interpretability, we rearrange the order of the results by percentage change

```{r}
#Arrange results from greater percentage change to lower
int.df <- model.metrics.df[1,]
factors.df <- arrange(model.metrics.df[-1,], desc(Predicted_Change))
ordered.model.metrics.df <- bind_rows(int.df, factors.df)
ordered.model.metrics.df
...
#Application of the function to our binomial model
perc.change.df <- f.metrics.binomial(binomial.model.c)
perc.change.df
```
